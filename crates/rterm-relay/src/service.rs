use crate::pty::{PtySpawner, RealPtySpawner};
use crate::session;
...
impl UnaryService<GetSnapshotRequest> for GetSnapshotSvc {
    ...
    fn call(&mut self, request: Request<GetSnapshotRequest>) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            let mut snap = crate::screen_diff::snapshot(lock.terminal.screen());
...
pub(crate) fn resolve_key(name: &str, app_cursor: bool) -> Option<Vec<u8>> {
